        ...
        for(int zCellPos=0; zCellPos<cellsInXDir;zCellPos++){
            for(int yCellPos=0; yCellPos<cellsInYDir;yCellPos++){
                for(int xCellPos=0; xCellPos<cellsInYDir;xCellPos++){
                    ...
                    for(int j=0; j< cellList[i]->atomsInCell.size(); j++){

                        //particles in own cell
                        //thisAtom = (j)->second;
                        thisIndex = cellList[i]->atomsInCell[j]->getSystemIndex();
                        //auto dummyk = j;
                        //advance(dummyk,1);
                        for(int k=thisIndex+1; k < cellList[i]->atomsInCell.size(); k++){
                            otherIndex = cellList[i]->atomsInCell[k]->getSystemIndex();
                        ...
                        for(int neighbour = 0; neighbour<cellList[i]->neighbourList.size(); neighbour++){
                            ...
                            for(int k=0; k!= cellList[neighbourPos]->atomsInCell.size(); k++){
                                otherIndex = cellList[neighbourPos]->atomsInCell[k]->getSystemIndex();
