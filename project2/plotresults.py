...
def plotsystemproperties():
	folder = 'MDnano-build-desktop-Qt_4_8_1_in_PATH__System__Release/experiments/gravitycyl/results/'
	N = len(glob.glob(folder + 'results*.xyz'))
...
def velocityProfile():
	fromexperiment = "gravitycyl"
	fromState = "MDnano-build-desktop-Qt_4_8_1_in_PATH__System__Release/experiments/" + fromexperiment + "/results/lastState.xyz"
	fromFile = open(fromState)
	totalAtoms = int(fromFile.readline())
	fromFile.readline()
	contents = range(totalAtoms)
	counter = 0
	for line in fromFile:
		contents[counter] = line.split(' ')
		counter += 1

	pos = np.zeros(totalAtoms)	
	for i in range(totalAtoms):
		pos[i] = float(contents[i][1])

	size = max(pos)
	origin = size/2.
	nBins = 40
	folder = "MDnano-build-desktop-Qt_4_8_1_in_PATH__System__Release/experiments/" + fromexperiment + "/results/"
	N = len(glob.glob(folder + 'results*.xyz'))
	...
	for i in range(N-100,N):
		...
		ax = np.linspace(0,rmax,nBins)
		for i in range(totalAtoms):
			if contents[i][10] == "0":
				r = (float(contents[i][2])-origin)**2 + (float(contents[i][3])-origin)**2
				r = np.sqrt(r)
				#print xpos
				n += 1
				...
				print bin
				if bin < nBins:
					vbins[bin] += float(contents[i][4])#*float(contents[i][4])
					nbins[bin] +=1
	...
	plt.ylabel('density (not normalized)')
	plt.xlabel('radial distance')
	plt.figure()
	plt.plot(ax,vbins,'r')
	plt.title('radial velocity distribution')
	plt.ylabel('relative occurance (not normalized)')
	plt.xlabel('radial distance')
	plt.show()
